...
async def test_get_user_info(ac: AsyncClient):
    ...
    async with test_db.async_session() as session:
        ...
        assert test_user.id == response.json()["user"]["id"], "Запрашиваемый id не совпадает с возвращаемым id"
...
async def test_create_user(ac: AsyncClient):
    ...
    response = await ac.post("api/users/?api_key=lll", json={
        "name": "LILY",
        "nickname": "WING_DEATH",
        "email": "L@capcom.com",
    })
    ...
    assert result.id == response.json()["id"], "Запрашиваемый id не совпадает с возвращаемым id"
...
async def test_follow(ac: AsyncClient):
    ...
    response = await ac.post(f"api/users/{test_user.id}/follow/?api_key={test_follower.token}")
...
async def test_unfollow(ac: AsyncClient):
    ...
    async with test_db.async_session() as session:
        ...
        response = await ac.delete(f"api/users/{test_user.id}/follow/?api_key={test_follower.token}")
        result = await session.scalar(query)
